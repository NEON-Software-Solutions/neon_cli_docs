...
Du möchtest ein zusätzliches Feature für die *[NEON Core App][neon_core_app_link]* entwickeln? Dann bist du hier genau richtig.
...
Es handelt sich also um ein zusätzliches Feature für die *[NEON Core App][neon_core_app_link]*, das heißt, du musst dir zuerst von der CLI ein neues Core-Projekt generieren lassen. Das bedeutet: bei den Optionen im Terminal genau NICHTS (oder wie Steven sagen würde: "Null Komma gar nichts") auswählen! So kann das Feature dann wunderbar von Grund auf getestet werden.
...
### Core App Brick updaten (?!)

Hat dein neues Feature Änderungen im ```core_app```-Brick verursacht (z.B. in ```lib/main.dart```, was ein File ist, dass das ```core_app```-Brick generiert)? Dann musst du jetzt ein paar Setup-Schritte durchführen. Wenn nicht, dann überspringe diesen Absatz.

Du hast gerade die *NEON Core App* mit einem Feature erweitert. Diese Erweiterung muss natürlich auch im ```core_app```-Brick gespiegelt werden. Füge dem ```core_app```-Brick als erstes eine Variable hinzu, die dein neues Feature kodiert. Beispiel: dein neues Brick heißt ```new_feature```. Füge der ```core_app/brick.yaml``` die Variable ```new_feature``` unter dem Bereich ```vars``` hinzu (mit ```type: boolean```, da es sich um ein Ein/Aus Toggle handelt und im Idealfall noch einer kleinen Beschreibung).

Hat die Entwicklung deines neuen Features Files außerhalb des Feature-Ordners verändert (z.B. einen neuen BlocProvider in ```main_app_loader.dart```)? Dann musst du weitere Änderungen am ```core_app```-Brick vornehmen. Sieh dir dazu sowohl ```hooks/pre_gen.dart```, als auch ```hooks/post_gen.dart``` des ```core_app```-Bricks an, die die Imports von ```app.dart``` und ```main_app_loader.dart``` verwalten, bzw. ein File-Cleanup Post-Gen durchführen. Und sollte das nicht reichen, dann ändere selbstverständlich auch andere Files im Brick. Der Bums muss natürlich laufen 🏃🏾!

Ich sage es nochmal, weil ich hier gerade um 3:34 Uhr morgens einen 💩-Bug fixe... Schau dir die Pre- und Post-Gen-Hooks vom ```core_app```-Brick an!!! Schreibt dein Brick irgendwas in ```injectable_module.dart```? Dann stelle sicher, dass es in der Bedingung für die Löschung davon im ```post_gen.dart``` mit drin steht! Analog dazu, falls es etwas in ```app_settings.dart``` schreibt!
...
Als ersten Schritt müssen wir das gleich machen, wie beim ```core_app```-Brick: dein Brick hieß ```new_feature``` und wir haben im ```core_app```-Brick eine Variable hinzugefügt, die exakt so heißt. Das musst du für das Pubspec Brick auch machen!!! (Wenn du einen Blick in die ```brick.yaml``` des ```pubspec```-Brick wirfst, wird dir die große Ähnlichkeit zu dem ```brick.yaml``` des ```core_app```-Bricks auffallen).
🚨🚨🚨
GANZ GANZ WICHTIG: Der Variablenname deines neuen Bricks MUSS im ```core_app```- und ```pubspec```-Brick IDENTISCH SEIN!🚨🚨🚨
...
```dart
'needs_go_router': vars['new_feature'], // Du erinnerst dich: 'new_feature' war der Name der Variable, die wir sowohl dem ```core_app```-, als auch dem ```pubspec```-Brick neu hinzugefügt hatten und die dein neues Feature kodiert!
```
...
- ```mason_var_name```: Dieser String ist SEHR wichtig, falls dein neues Feature eine Änderung im ```core_app```- und/oder ```pubspec```-Brick verursacht hat. Unser Beispiel von vorhin (```new_feature```) hat das getan, also muss der Rückgabewert von ```mason_var_name``` *EXAKT* der Name der Variable sein, die wir im ```core_app```- und/oder ```pubspec```-Brick vorhin definiert hatten! Sollte dein Brick weder ```core_app```-, noch ```pubspec```-Brick verändern (wie z.B. das ```mason_integration_feature```-Brick), dann gib hier den leeren String ```''``` zurück.
...
- ```getBrickGenerationDirectoryPath```: Der Pfad zu dem Verzeichnis, in dem dein Brick generiert werden soll. Dein Brick ist ein Feature? Dann würde ```projectDirectoryPath/lib/features``` Sinn machen. Du hast einen neuen Core-Service (wie z.B. ```ToastService```) implementiert? Dann gib ```projectDirectoryPath/core/services``` zurück. Wenn dein Brick gar keine neuen Files generiert (sondern z.B. nur Files aus dem ```core_app```-Brick ändert), dann gib hier den leeren String zurück.
- ```brickPath```: Der Pfad, unter dem dein neues Brick in dem Repo hier gespeichert ist (ausgehend vom Ordner ```NEON_bricks/cli_bricks/bricks```). Für das ```core_app```-Brick wäre der Rückgabewert von ```brickPath``` einfach ```core_app```. Sollte dein neues Brick keine neuen Files generieren, gib auch hier wieder den leeren String zurück.
...
- ```generationErrorMessage```: Die Fehlermeldung, die ausgegeben werden soll, falls bei Generierung deines Bricks etwas schiefgeht. Dein Feature generiert keine eigenen Files (sondern ändert nur Files z.B. im ```core_app```-Brick)? Leeren String zurückgeben.
